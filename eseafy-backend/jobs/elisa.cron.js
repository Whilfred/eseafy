...
// ══ CONFIG SMTP ══
// Remplace par tes vraies credentials
        ...
        <div style="max-width:560px;margin:0 auto;font-family:'Helvetica Neue',sans-serif;color:#111;">
          ...
          <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;padding:28px 32px;border-radius:0 0 12px 12px;">
            ...
            <p style="font-size:12px;color:#9ca3af;text-align:center;margin:0;">
              ${boutique_nom} · Propulsé par Easyfy
            </p>
          </div>
        </div>`
    },
...
async function envoyerCampagnesElisa() {
  ...
  try {
    // Récupérer toutes les campagnes programmées non encore envoyées
    ...
    for (const camp of campagnes.rows) {
      try {
        ...
        if (destinataires.length === 0) {
          await pool.query(
            `UPDATE elisa_campagnes SET statut = 'vide' WHERE id = $1`,
            [camp.id]
          );
          continue;
        }
        ...
        for (const dest of destinataires) {
          ...
          try {
            ...
            // Petite pause pour éviter le spam throttling
        ...
        await pool.query(
          `UPDATE elisa_campagnes 
           SET statut = 'envoyee', envoyes = $1, echecs = $2, sent_at = NOW()
           WHERE id = $3`,
        ...
        await pool.query(
          `UPDATE elisa_campagnes SET statut = 'erreur' WHERE id = $1`,
          [camp.id]
        );
      }
    }
...
// ══ PLANIFIER ══
// Toutes les 6h — vérifie s'il y a des campagnes à envoyer
