...
const { sendEmailBienvenue, sendNotificationInscription, sendOTP, sendPasswordChanged } = require('../services/emailService');
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    // Vérifier l'OTP
    if (!user.otp_code || !user.otp_expire_at || user.otp_code !== otp) {
...
router.post('/change-password', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!user.password) {
      return res.status(400).json({ 
        message: 'Ce compte utilise Google. Utilisez "Ajouter un mot de passe" depuis votre profil.' 
      });
    }
