...
`, [userId]);
    ...
    const ventesGraph = await pool.query(`
      SELECT
    ...
    const statuts = {
      paye: { total: 0, nb: 0 },
      en_attente: { total: 0, nb: 0 },
      rembourse: { total: 0, nb: 0 },
      annule: { total: 0, nb: 0 }
    };
    ...
    revenusParStatut.rows.forEach(r => {
      if (statuts[r.statut]) {
        statuts[r.statut].total = parseFloat(r.total);
        statuts[r.statut].nb = parseInt(r.nb);
      }
    });

    totauxGlobaux.rows.forEach(r => {
      if (globaux[r.statut]) {
        globaux[r.statut].total = parseFloat(r.total);
        globaux[r.statut].nb = parseInt(r.nb);
      }
    });
    ...
    return res.json({
      ...
      graph_ventes: ventesGraph.rows, // ✅ AJOUT ICI
