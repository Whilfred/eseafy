...
transporter.verify((err, success) => {
  ...
  else     console.log('✅ Email service connecté (Brevo)');
});
...
async function sendConfirmationCommande({ email, nom_client, reference, produit, montant, boutique_nom, vendeur_tel }) {
  ...
  const content = `
    ...
    <p style="color:#6a6760;font-size:13.5px;margin:0 0 20px;">Le vendeur vous contactera très bientôt pour organiser la livraison. Conservez votre référence de commande.</p>
    ...
    <div style="text-align:center;">
      <a href="${waUrl}" style="display:inline-block;background:#25d366;color:#ffffff;padding:12px 24px;border-radius:10px;text-decoration:none;font-size:13px;font-weight:600;">💬 Contacter le vendeur sur WhatsApp</a>
    </div>` : ''}`;
...
async function sendNotificationVendeur({ email_vendeur, nom_vendeur, reference, produit, montant, nom_client, telephone_client }) {
  ...
  try {
    ...
    console.log(`📧 Email vendeur envoyé à ${email_vendeur}`);
  } catch (err) {
    console.error('❌ Erreur envoi email vendeur :', err.message);
  }
}
...
async function sendEmailMarketing({ email, nom, sujet, contenu_html, boutique_nom }) {
  ...
  const content = `
    ...
    <div style="color:#6a6760;font-size:14px;line-height:1.7;">
      ${contenu_html}
    </div>`;
    ...
    <div style="background:#f0f5ff;border:1px solid #e8f0ff;border-radius:12px;padding:20px;margin-bottom:24px;">
      ...
      <div style="display:flex;flex-direction:column;gap:8px;">
        ${['📦 Créer vos premiers produits', '🔗 Partager le lien de votre boutique', '📊 Suivre vos ventes en temps réel', '🎟 Créer des codes promo pour attirer des clients'].map(item => `
          <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:#ffffff;border-radius:8px;font-size:13px;">
            ${item}
          </div>`).join('')}
      </div>
    </div>
